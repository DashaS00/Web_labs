		...
		onDrop(evt, list) {
			...
			for (let i = 0; i < this.tasks.length; i++) {
				...
				if (this.tasks[i]['dateEnd'] === "" && this.tasks[i]['status'] === "done") {
					...
					m = (lot / (1000 * 60)) % 60;
					s = (lot / 1000) % 60;
					this.tasks[i]['dateAll'] = this.zeroPrefix(d) + Math.trunc(d) +" дней " +
						this.zeroPrefix(h) + Math.trunc(h) + " часов " +
						this.zeroPrefix(m) + Math.trunc(m) + " минут " +
						this.zeroPrefix(s) + Math.trunc(s) + " секунд"
				}
			}
		},
		zeroPrefix(num) {
			...
			if ((this.obj.trust && !this.obj.status) ||
				...
				for (i in this.tasks) {
					if (this.tasks[i]['local_name'] == this.obj.naming) {
						...
						m = (lot / (1000 * 60)) % 60;
						s = (lot / 1000) % 60;
						this.tasks[i]['dateAll'] = Math.trunc(d) +" дней "
							 + Math.trunc(h) + " часов "
							 + Math.trunc(m) + " минут "
							 + Math.trunc(s) + " секунд"
					}
